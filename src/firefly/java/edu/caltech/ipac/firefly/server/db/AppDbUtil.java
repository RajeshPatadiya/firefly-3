...
public class AppDbUtil {
    ...
    private static final String META_TABLE_SQL = "create table if not exists %s "+
            "(" +
            "  cname    varchar(255)" +
            ", label    varchar(255)" +
            ", type     varchar(255)" +
    ...
    private static final String REQ_TABLE_SQL = "create table if not exists %s" +
            " (" +
...
//        String[] initSql = {"SET CACHE_SIZE 262144", "SET LOG 0", "SET UNDO_LOG 0"};
//        for (String sql : initSql) getAppDb().execute(sql);

        String sql = String.format(REQ_TABLE_SQL, TBL_REQUEST_TNAME);
        logger.debug("initApplicationDb sql:" + sql);
        getAppDbSimple().update(sql);

        sql = "create sequence if not exists " + REQID_SEQ + " start with 0";
        logger.debug("initApplicationDb sql:" + sql);
        getAppDbSimple().update(sql);
    }
    ...
    public static long storeTable(DataGroup dg) {
        ...
        long reqId = getAppDbSimple().queryForLong("call NEXT VALUE FOR reqid_seq");
...
//====================================================================
    ...
    private static void createDataTbl(String dataTblName, DataGroup dg) {
        ...
        String sqlCreate = String.format("create table if not exists %s (", dataTblName);
