...
public class DbDemoProcessor implements SearchProcessor<DataGroupPart> {
    ...
    public DataGroupPart getData(ServerRequest request) throws DataAccessException {
        ...
        try {
            ...
            StopWatch.getInstance().start("DataGroupWrite");
            DataGroupWriter.write(File.createTempFile("test", ".tbl", ServerContext.getPermWorkDir()), dg);
            StopWatch.getInstance().stop("DataGroupWrite").printLog("DataGroupWrite");
